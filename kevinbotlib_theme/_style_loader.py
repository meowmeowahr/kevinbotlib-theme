...
from kevinbotlib_theme import __version__, _os_appearance, _resources
from kevinbotlib_theme._template import filter
from kevinbotlib_theme._template.engine import Template
from kevinbotlib_theme._util import get_cash_root_path, get_logger
...
def _detect_system_theme(default_theme: str) -> str:
    ...
    if system_theme is None:
        _logger.info(f'failed to detect system theme, kevinbotlib_theme use "{default_theme}" theme.')
    ...
    *,
    ...
    Examples:
        ...
        1. Dark Theme ::
            ...
            app = QApplication([])
            app.setStyleSheet(kevinbotlib_theme.load_stylesheet())
            # or
        ...
        4. Sharp corner ::
            ...
            app = QApplication([])
            app.setStyleSheet(kevinbotlib_theme.load_stylesheet(corner_shape="sharp"))

        5. Customize color ::

            app = QApplication([])
            app.setStyleSheet(kevinbotlib_theme.load_stylesheet(custom_colors={"primary": "#D0BCFF"}))

        6. Customize a specific theme only ::

            app = QApplication([])
            app.setStyleSheet(
                kevinbotlib_theme.load_stylesheet(
                    theme="auto",
                    custom_colors={
                        "[dark]": {
                            "primary": "#D0BCFF",
    ...
    try:
        from kevinbotlib_theme.qtpy.QtCore import QCoreApplication

        app = QCoreApplication.instance()
        if app is not None and not app.property("_kevinbotlib_theme_use_setup_style"):
    ...
    *,
    ...
    Examples:
        ...
        1. Dark Theme ::
            ...
            app = QApplication([])
            app.setPalette(kevinbotlib_theme.load_palette())
            # or
            app.setPalette(kevinbotlib_theme.load_palette("dark"))

        2. Light Theme ::

            app = QApplication([])
            app.setPalette(kevinbotlib_theme.load_palette("light"))

        3. Automatic detection of system theme ::

            app = QApplication([])
            app.setPalette(kevinbotlib_theme.load_palette("auto"))
