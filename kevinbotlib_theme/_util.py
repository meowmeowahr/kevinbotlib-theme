"""Utility methods for kevinbotlib_theme."""
...
def get_cash_root_path(version: str) -> Path:
    ...
    return Path.home() / ".cache" / "kevinbotlib_theme" / f"v{version}"


def get_kevinbotlib_theme_root_path() -> Path:
    """Return the kevinbotlib_theme package root path.

    Returns:
        kevinbotlib_theme package root path.
    """
    return Path(inspect.getfile(kevinbotlib_theme)).parent
